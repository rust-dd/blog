    ...
    rsx! {
        ...
        SuspenseBoundary {
            ...
            div { class: "w-full font-mono",
                section { class: "py-4",
                    p { class: "text-xs text-slate-400", "// portfolio" }
                    h1 { class: "mt-2 text-3xl font-semibold leading-tight text-slate-900 sm:text-4xl md:text-5xl", "Built Systems" }
                            ...
                            rsx! {
                                ...
                                div { class: "mt-4 border-y border-dashed border-slate-300 py-3 text-xs text-slate-500",
                                    div { class: "flex flex-wrap gap-x-4 gap-y-1",
                                        span { "projects: " span { class: "text-slate-700", "{items.len()}" } }
                                        span { class: "hidden sm:inline", "|" }
                                        span { "stack: " span { class: "text-slate-700", "rust" } }
                                        span { class: "hidden sm:inline", "|" }
                                        span { "latest: " span { class: "text-slate-700", "{latest_year}" } }
                                    }
                                }
                                ...
                                if !categories.is_empty() {
                                    div { class: "mt-4 text-xs text-slate-500",
                                        ...
                                        span { class: "text-slate-500", "categories" }
                                ...
                                section { class: "mt-6 grid gap-4 md:gap-5 lg:grid-cols-2",
                                    for (index, reference) in items.iter().enumerate() {
                                        article { class: "group rounded-lg border border-slate-200 bg-white p-4 transition-colors duration-200 hover:border-slate-400 sm:p-5",
                                            ...
                                            h2 { class: "mt-3 text-lg font-semibold leading-tight text-slate-900", "{reference.title}" }
                                            ...
                                            if !reference.tags.is_empty() {
                                                p { class: "mt-2 text-xs text-slate-400",
                                                    {reference.tags.iter().take(8).map(|t| format!("#{t}")).collect::<Vec<_>>().join(" ")}
                                                }
                                            }
                                            ...
                                            if !reference.url.is_empty() {
                                                a {
                                                    ...
                                                    class: "mt-3 inline-flex items-center gap-1 text-xs text-slate-500 transition-colors duration-200 hover:text-slate-700",
                                                    "open"
                                                    span { ">" }
                                                }
                                            }
